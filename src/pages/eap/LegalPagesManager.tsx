...
export default function LegalPagesManager() {
  const [pages, setPages] = useState<LegalPage[]>(seed);
  ...
  const deletePage = (id: string) => {
    ...
    toast({ title: "Deleted", description: "Page removed.", variant: "destructive" });
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage Privacy Policy, Terms, Refund & other pages for each website.
            </p>
          </div>

          <div className="flex items-center gap-2">
            <Select value={website} onValueChange={(v) => setWebsite(v as Website)}>
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-6 text-center text-muted-foreground">
                        No pages found for <span className="font-medium capitalize">{website}</span>.
                      </td>
                    </tr>
                  ) : (
                    filtered.map((p) => (
                      <tr key={p.id} className="border-t">
                        ...
                        <Td className="text-right pr-4">
                          <div className="inline-flex items-center gap-2">
                            <Button size="icon" variant="ghost" title="View" onClick={() => setOpenView(p)}>
                              <Eye className="h-4 w-4" />
                            </Button>
                            <Button size="icon" variant="ghost" title="Edit" onClick={() => setOpenEdit(p)}>
                              <Pencil className="h-4 w-4" />
                            </Button>
                            <Button size="icon" variant="ghost" title="Delete" onClick={() => deletePage(p.id)}>
        ...
        <LegalPageFormDialog
          ...
          onSave={(payload) => {
            ...
            const created: LegalPage = {
              id: `lp_${Math.random().toString(16).slice(2)}`,
              ...payload,
              updatedAt: now,
            };
        ...
        <LegalPageFormDialog
          ...
          onSave={(payload) => {
            ...
            const now = new Date().toISOString();
            setPages((prev) =>
              prev.map((x) => (x.id === openEdit.id ? { ...x, ...payload, updatedAt: now } : x))
            );
            setOpenEdit(null);
            ...
            {openView && (
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-xl font-semibold">{openView.title}</div>
...
function LegalPageFormDialog({
  ...
  const [type, setType] = useState<PageType>(initial?.type ?? "privacy_policy");
  ...
  const submit = () => {
    ...
    if (!slug.trim().startsWith("/")) {
      toast({ title: "Invalid slug", description: "Slug must start with / (example: /privacy-policy)." });
      return;
    }
    if (!content.trim()) {
      toast({ title: "Missing content", description: "Content is required." });
      return;
    }
      ...
      <DialogContent className="max-w-4xl">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-3">
            ...
            <div>
              <Label>Title*</Label>
              <Input value={pageTitle} onChange={(e) => setPageTitle(e.target.value)} placeholder="Privacy Policy" />
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Example: <span className="font-mono">/terms</span>, <span className="font-mono">/refund-policy</span>
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          <Button onClick={submit}>{initial ? "Save Changes" : "Create Page"}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
